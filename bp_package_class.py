...
class Ball_Python_Package:
  ...
  def combine_bp_packages(self, package1, package2):
    ...
    for snake in package2.snakes:
      self.snakes.append(snake)
    #getting and adding trait value for each snake to pack's value, and adding traits to packate_traits if they are not already in the list
    for snake in self.snakes:
      for trait in snake["Traits"]:
        trait_value = self.get_trait_value(trait)
        self.pack_trait_value += trait_value
        if trait not in self.package_traits:
          self.package_traits.append(trait)
    #counting number of different (unique) traits included in the package
    for trait in self.package_traits:
      self.pack_trait_count += 1
  ...
  def get_trait_value (self, trait):
    ...
    recessive_pos_het_t = ['50% het lavender', '50% het piebald', '50% het clown', '50% het hypo', '66% het lavender', '66% het piebald', '66% het clown', '66% het hypo']
    t_not_genetic_or_low_val = ['pet only', 'paradox', 'pos het lavender', 'pos het piebald', 'pos het clown', 'pos het hypo']
    ...
    if trait in recessive_visual_t:
      ...
      t_value = 1
    elif trait in t_not_genetic_or_low_val:
      t_value = 0
    else:
      t_value = 2
    return t_value